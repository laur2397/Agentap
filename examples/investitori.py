...
STARE=(
 ...
 "IMBUNATATIRI R3 (verificate cu Playwright + curl pe localhost): "
 "(e) ENFORCEMENT Pro REAL server-side: releul emite un TOKEN semnat HMAC; functiile Pro (multi-device) sunt verificate de server (HTTP 402 daca "
 "token-ul lipseste/e falsificat/expirat) — Pro NU mai e 'simbolic'/pirateabil pe client. Demonstrat: token falsificat respins, valid acceptat. "
 "Plata in sine ramane stub (in prod: webhook Stripe -> grant token); arhitectura de enforcement e insa reala. "
 "(f) ANTI-ABUZ/SYBIL in discovery: rate-limiting pe releu la publicare (max 10/10min/IP -> 429), plus 'Raporteaza/Blocheaza' pe client (lista de blocati, "
 "profilele blocate dispar din cautari). Demonstrat: a 11-a publicare respinsa. "
 "(g) RECUPERARE date/cont: backup criptat .eie + transfer pe alt dispozitiv = cale reala de recuperare la pierderea telefonului (identitatea ECDH se "
 "transfera in .eie), deci 'pierderea cheilor' e recuperabila daca userul isi face backup. "
