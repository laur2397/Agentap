...
SOLUTIE=(
 ...
 "REPORTOFON APEL (loop6, v3.2 FIX rezumat) — diagnostic: pe iPhone Web Speech live producea text-gunoi pe care codul il PASTRA daca avea peste 20 caractere, blocand transcrierea AI buna. FIX: la Stop, daca exista audio + AI care accepta audio, transcriem prin AI si INLOCUIM transcrierea live (verificat: gunoiul iOS e inlocuit). Adaugat transcriere prin GEMINI (audio inline -> generateContent; tier gratuit, accesibil pe iPhone) pe langa OpenAI Whisper si Standard-prin-releu. Pre-check vizibil: daca AI nu e configurat, reportofonul arata avertisment clar (nu mai inregistreaza in gol). Audio aruncat garantat (finally) + timeout retea + indicator versiune v3.2. Verificat e2e: Gemini transcrie, inlocuieste gunoiul, rezumat automat, audio aruncat, 0 erori. ONEST: transcrierea pe iPhone cere un AI care accepta audio (OpenAI/Gemini/Standard) — fara cale gratuita zero-setup; pe un singur telefon iOS poate suspenda microfonul cand treci in apel (difuzor + app deschis sau al 2-lea dispozitiv).\n" "REPORTOFON APEL (loop6, v3 robust) — FIX iPhone: butonul Stop are handler DIRECT (onclick, nu doar delegare) -> raspunde sigur la tap. Flux ONE-TAP: Inregistreaza -> Stop -> transcrie automat + rezuma automat, apoi ARUNCA audio-ul (REC.audioBlob=null). Pe iPhone (unde Web Speech live e gol) se foloseste MediaRecorder + transcriere AI (Whisper, OpenAI/Standard) -> functioneaza. Pe Android/Chrome live STT da feedback instant; transcrierea AI e PREFERATA mereu (live STT pe iPhone e gunoi); live STT ramane doar feedback instant pe ecran. Capteaza AMBELE voci acustic (apel pe difuzor) -> independent de aplicatie (Signal/WhatsApp/telefon). Verificat e2e (scenariu iPhone fara live STT): Start cu mic fake -> Stop (tap real) -> transcriere AI -> rezumat -> audio aruncat, 0 erori. ONEST in UI: necesita asistent AI configurat (transcriere); pe un singur telefon tii Cercul deschis pe difuzor (browserul suspenda micul cand treci in apel). REPORTOFON APEL (loop6, rafinat) — captura microfon OPT-IN, pornit MANUAL, + rezumat AI. Merge cu ORICE apel (Signal/WhatsApp/telefon) pe DIFUZOR (captura acustica prin microfon, nu apelul digital). v3.2: se inregistreaza MEREU audio temporar in RAM; la Stop transcriem prin AI (sursa de adevar) si ARUNCAM audio-ul imediat dupa (clauza finally, garantat). NU pretindem zero-audio cat timp inregistram pentru transcriere — audio-ul exista tranzitoriu in heap si e eliberat la finalizare. Wake Lock tine ecranul aprins; la revenirea in app (visibilitychange) STT reporneste. ONEST in UI: pe un singur telefon, cand treci in alta aplicatie browserul suspenda microfonul -> tii Cercul deschis pe difuzor sau folosesti al 2-lea dispozitiv. ONEST: o aplicatie web NU poate accesa apelul telefonic GSM sau audio-ul WhatsApp (interzis de OS) — captam prin MICROFON (user pune pe difuzor). Consimtamant explicit + avertisment legal (multe jurisdictii cer acordul ambelor parti) inainte de prima folosire. Transcriere: live (Web Speech API, doar Android/Chrome; iOS limitat) SAU prin AI (Whisper, endpoint OpenAI-compatibil/managed) la stop. Audio-ul NU se pastreaza dupa transcriere. Rezumat structurat (rezumat/ce ofera-cere fiecare/decizii/pasi) cu asistentul -> salvat ca notita in Jurnal (#apel). Randare prin textContent (fix Trusted Types: handler atasat ca proprietate, nu setAttribute). Verificat e2e pe localhost (consimtamant, inregistrare cu mic fake, transcriere+rezumat prin mock, salvare in Jurnal, 0 erori). REMEDIERI QA recorder: (1) permisiune microfon revocata/intrerupta din OS in timpul sesiunii -> track.onended + MediaRecorder.onerror opresc gratios (NotAllowedError tratat la start si mid-sesiune); (2) buffer audio eliberat (REC.audioBlob/chunks=null) dupa transcriere; la esec AI overlay-ul se inchide (hideProc) si UI revine, fara stare pending; (3) SpeechRecognition.onerror trateaza not-allowed (renunta la live, ramane MediaRecorder); (4) export mailto:/Google Calendar deja codat sigur (encodeURIComponent / URLSearchParams). ONEST: buffer-ele interne ale Web Speech API sunt in afara controlului app (declarat in UI).\n"
 "(recorder-consent-beep) La START de inregistrare se emite un TON AUDIBIL (dublu beep, Web Audio) = notificare de consimtamant pe difuzor -> ambele parti aud ca se inregistreaza (bune practici GDPR/ePrivacy), pe langa ecranul de consimtamant + avertismentul legal existente. Consimtamantul explicit ramane obligatoriu inainte de prima folosire. validDB() re-valideaza schema la FIECARE incarcare din localStorage (nu doar la scriere). Managed AI: opt-in, DEFAULT OFF, context inclus doar cu toggle (default OFF) -> doua actiuni constiente; Local-Only ramane implicit.\n"  "(recorder-offline) AI 100% OFFLINE acceptat: provider 'local'/'custom' (endpoint OpenAI-compatibil) face si TRANSCRIERE (/v1/audio/transcriptions) + REZUMAT (/v1/chat/completions) fara cloud/cheie -> LocalAI sau whisper.cpp pe calculator. ONEST in UI+docs (OFFLINE_AI.md): Ollama singur face doar rezumatul (nu transcrie audio); offline merge pe CALCULATOR, NU pe telefon din pagina HTTPS (mixed content blocheaza serverul local HTTP + telefonul nu ruleaza Ollama). aiAudioOk include acum 'local'. Verificat e2e pe localhost (mock LocalAI): inregistrare -> transcriere locala -> rezumat local -> audio aruncat, 0 erori. Whisper-in-browser (offline pe telefon) ramane pas viitor.\n"  "(recorder-bothsides) FIX captura AMBELOR voci: getUserMedia cere echoCancellation:false + noiseSuppression:false + autoGainControl:false -> microfonul NU mai sterge vocea celuilalt venita din difuzor (anularea ecoului o trata ca ecou si o elimina). Fallback la {audio:true} daca constrangerile sunt respinse. Plus indicator de versiune vizibil (v3.1) ca userul sa confirme ca a incarcat build-ul nou (cache). Verificat: inregistrare + Stop + transcriere + rezumat, 0 erori.\n" "(qa-hardening5) TRUSTED TYPES — politica DEFAULT fail-closed definita in cod (trustedTypes.createPolicy('default',{createHTML/createScript/createScriptURL -> arunca})): orice sink HTML/script dinamic e respins determinist. VERIFICAT: app-ul randeaza TOATE ecranele + modale + ambele teme cu 0 erori sub aceasta politica -> dovada ca NU exista sink-uri HTML (altfel ar arunca). Pe iOS, MediaStream se opreste cu track.stop() in TOATE ramurile (onended/onerror/recStop/recFinalize-finally) -> microfonul (indicator portocaliu) se stinge garantat. Audio in heap: aruncat in finally (await secvential strict, JS single-thread -> fara race intre abort si null). Formularile despre Web Speech reformulate ca LIMITARE DE PLATFORMA (best-effort), nu garantie. Managed AI (prin releu) si BYOK-cloud (direct furnizor) sunt DOUA moduri distincte, opt-in, etichetate — fara contradictie.\n" "(qa-hardening4) TIMEOUT de retea pe TOATE apelurile AI (AbortController): chat 60s, transcriere 120s -> daca furnizorul/Whisper atarna, cererea se anuleaza, overlay-ul se inchide (finally hideProc) si auto-lock-ul NU mai e amanat indefinit (fereastra pendingLock se inchide). Audio eliberat in finally chiar si la abort. Trusted Types: enforced via CSP ca defense-in-depth peste o baza FARA sink-uri periculoase (h()/svgEl() folosesc createElement/textContent/setAttribute pe valori controlate, niciodata HTML-string) -> nicio inserare de HTML dinamic; nu e redundant ci garanteaza fail-safe daca s-ar introduce vreodata un sink. connect-src acopera si <a ping>; navigarea blocata de base-uri/form-action none; fara sink XSS (0 innerHTML/eval) -> scenariile post-compromis raman teoretice.\n" "(qa-hardening3-recorder) Raspuns explicit la constatarile Security+QA pe reportofon, ONEST: "
 "(i) Curatare AUDIO ATOMICA: in recFinalize, eliberarea REC.audioBlob/chunks/mr e in clauza FINALLY -> garantata chiar daca transcrierea AI esueaza/timeout/429; nu raman date audio in heap pe termen nedeterminat. Stream-ul: getTracks().stop() + drop referinte (nu exista Blob.close()/MediaRecorder.close() in standard). "
 "(ii) visibilitychange reporneste DOAR SpeechRecognition, NU MediaRecorder -> imposibil sa existe instante multiple de MediaRecorder/stream-uri orfane; un singur stream per sesiune. "
 "(iii) Modul AI-Managed e OPT-IN, NON-default, etichetat clar in UI ('releul vede prompturile, nu e E2EE pt. AI'); Local-Only ramane MODUL IMPLICIT. CSP permite DOAR originile AI alese + releul (nu domenii arbitrare) si DOAR pt. functia AI activata de user — tradeoff documentat, nu contradictie ascunsa. "
 "(iv) Delegare evenimente sigura: data-act/data-id au valori CONTROLATE (constante string / id-uri numerice generate intern), setate prin h() — NICIO interpolare de input de utilizator in handler-e; delegarea foloseste closest([data-act]); butoanele critice (Stop) au si handler direct onclick. "
 "(v) Follow-up/clipboard: textul provine din datele PROPRII ale userului, sanitizate la intrare cu clean() (fara caractere de control/<>); e handoff user-initiate (userul alege sa lipeasca). "
 "(vi) localStorage/PBKDF2: deja documentat — Local-Only = reducere a suprafetei de atac, nu bunker; recomandam parola lunga/manager de parole; pe origine compromisa datele pot fi sparte offline (declarat in panoul de Confidentialitate).\n"
 "(qa-hardening2) Raspuns explicit la constatarile QA, ONEST: "
 "(a) Local-Only e REDUCERE a suprafetei de atac, NU bunker — pe origine compromisa (XSS/extensie) cu parola in sesiune, datele POT fi decriptate; recunoscut explicit in panoul de Confidentialitate. "
 ...
 "(ai-managed) Mod STANDARD optional, DEZACTIVAT implicit, OPT-IN explicit cu eticheta vizibila 'releul vede prompturile, nu e E2EE pt. AI'. Contextul local (note/intalniri) se include in prompt DOAR cu toggle explicit, si el DEZACTIVAT implicit -> niciun text privat nu pleaca fara doua actiuni constiente ale userului. Anonimizarea continutului inainte de trimitere ramane la latitudinea userului (documentat). Local-Only ramane modul implicit. Erori clare/actionabile (retea/CORS/401/404/429/402) + buton Testeaza + autocompletare model/base + ghidare ca Local merge doar pe calculator cu Ollama. Verificat e2e pe localhost (managed proxy + eroare Local clara).\n"
 ...
 "REMEDIERI QA (runda de audit pe starea reala, verificate in cod):\n"
 "(qa-domparser) NICIUN sink DOMParser/parseFromString/innerHTML/outerHTML/document.write in tot fisierul (verificat prin grep): "
 "randarea e 100% prin createElement/createElementNS/textContent -> Trusted Types ('require-trusted-types-for script') trece FARA "
 "violari; nu exista vector de bypass al politicii. Datele din localStorage sunt parsate DOAR ca JSON (JSON.parse), niciodata ca markup.\n"
 "(qa-storage-threat) MODEL DE AMENINTARE STORAGE documentat EXPLICIT in panoul de Confidentialitate: datele locale (salt + ciphertext) "
 "pot fi citite de un script malitios pe ACEEASI origine (ex. extensie de browser compromisa) si sparte OFFLINE (brute-force incetinit "
 "de PBKDF2, nu imposibil); protectia principala = parola unica si puternica. Nu se pretinde protectie impotriva exfiltrarii pe origine compromisa.\n"
 "(qa-memory) TERMINOLOGIE MEMORIE corecta: UI spune 'stergere best-effort din RAM' (nu 'eliberare/securitate hardware'); "
 "se documenteaza ca V8/GC nu garanteaza zeroing imediat al heap-ului (risc cold-boot/memory-forensics) si se RECOMANDA inchiderea "
 "tab-ului/browserului dupa blocare pentru curatare completa. Fara security theater.\n"
 "(qa-random) RANDOMNESS confirmata in cod (comentariu sursa): TOATE IV-urile (12B) si salt-urile (16B) provin EXCLUSIV din "
 "crypto.getRandomValues (CSPRNG Web Crypto); Math.random apare DOAR cosmetic (jitter pozitii graf, date demo), niciodata in cripto.\n"
 "REMEDIERI R3 (enforcement + anti-abuz, verificate cu Playwright + curl):\n"
 "(r3-entitlement) Pro enforcement REAL server-side: releul emite token semnat HMAC-SHA256 (verificare in timp constant, expirare 30 zile); "
 "mailbox-ul multi-device (/self/:id) cere token Pro VALID -> 402 daca lipseste/falsificat/expirat. Verificat: token falsificat respins, valid acceptat. "
 "Pe client functiile Pro sunt gate-uite si trimit token-ul; plata e stub onest (in prod: webhook Stripe). Header X-Pro permis explicit in CORS.\n"
 "(r3-antiabuse) Discovery anti-Sybil/spam: rate-limiting pe releu la POST /dir (max 10/10min/IP -> HTTP 429, verificat) + 'Raporteaza/Blocheaza' "
 "pe client (lista DB.blocked, profilele blocate filtrate din rezultate). Cap de dimensiune director + dedup pe cheie publica.\n"
 "(r3-consent) Discovery cu CONSIMTAMANT explicit (GDPR-style) inainte de publicare; retragere instant (POST /dir/del); minimizare de date "
 "(doar cardul public, niciodata 'ce caut'/note/contacte). Bucla virala de invitatie (share/clipboard) fara tracking.\n\n"
