...
- 2026-06-01 · M3b FREEMIUM (verificat) · RASPUNS la obiectia #3 (monetizare): model de tier-uri Free/Pro + gating client-side + UI de plan in Profil. Free = Local-Only+backup pe veci, sync E2EE cu max 3 contacte, discovery; Pro = sync nelimitat + multi-device (self-sync gated de isPro) + prioritate. syncNow plafoneaza la 3 peers pe Free si dezactiveaza self-sync. ONEST: upgrade-ul e DEMO (fara plata reala); UI spune clar ca enforcement-ul real cere token de drept server-side + Stripe. VALIDAT Playwright (default Free, upgrade->Pro schimba limitele, downgrade->Free), 0 erori.
